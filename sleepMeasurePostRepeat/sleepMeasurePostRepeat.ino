...
#ifdef ADCCALIBRATION
#include "ADCCalibration.h"
#endif //ADCCALIBRATION
...
float getADC(int pin) {
  ...
  if (! isnan(value)) {
    #if defined(ADCCALIBRATION) and defined(ESP32)
    return getCalibratedADC(value / NADCREADINGS);
    #else
    return value / NADCREADINGS;
    #endif //ADCCALIBRATION and ESP32
  }
  else return std::numeric_limits<double>::quiet_NaN();
}
...
void postValues() {
  ...
  //------------------------------------------
  //MQTT message
  StaticJsonDocument<MQTTMESSAGESIZE> doc;
  getMQTTMessage(doc);
...
void setDocTRH(JsonDocument &doc, float t, float rh) {
  ...
  if ( ! isnan(getDewPoint(t, rh))) {
    ...
    if (t > getDewPoint(t, rh)) doc["dewpstatus"]=1;
